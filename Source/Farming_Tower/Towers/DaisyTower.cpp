...
void ADaisyTower::Shoot_Implementation()
{
    ...
    for (AActor* Enemy : EnemyList)
    {
        ...
        if (bHit && HitResult.GetActor() == Enemy)
        {
            ...
            DrawDebugLine(GetWorld(), StartLocation, EndLocation, FColor::Yellow, false, .5f, 0, 2.f);
    ...
    // Schedule second shot after 0.25 seconds
    GetWorldTimerManager().SetTimer(
        SecondShotTimerHandle,
        this,
        &ADaisyTower::ShootSecondShot,
        0.25f,
        false
    );
}

void ADaisyTower::ShootSecondShot()
{
    if (!ProjectileClass || !TowerRangeComponent) return;

    TArray<AActor*> EnemyList = TowerRangeComponent->GetSortedEnemiesInRangeByEndProgress();
    FVector StartLocation = GetActorLocation() + FVector(0, 0, SpawnHeightOffset);
    FCollisionQueryParams TraceParams(FName(TEXT("DaisyTraceSecondShot")), true, this);
    TraceParams.AddIgnoredActor(this);
